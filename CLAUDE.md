...
### Container layout (`backend/Dockerfile` + `render.yaml`)
Docker build context is the **repo root** (`dockerContext: .`), and the image mirrors the repo tree: `/app/backend/app`, `/app/scripts/config`, `/app/uploads`; `CMD` runs `uvicorn app.main:app --app-dir backend`. This is load-bearing — `services/cover_art.py` and `routers/posts.py` locate `scripts/config/*.json` via `Path(__file__).resolve().parents[3]`, and `uploads.py` uses `parents[2]`. Flattening `app/` into `/app` makes all of them resolve under `/` and the cover-art / topic-presentation configs silently fall back to built-in defaults. `APP_ENV=production` is set in both the Dockerfile and `render.yaml` so the image never depends on Render-injected vars to know it is in production (otherwise `auth.py` boots with `admin`/`admin123` and the default JWT secret). Root `.dockerignore` keeps `.env*`, `frontend/`, `node_modules`, tests and local DBs out of the build context.
...
Five routers mounted in `main.py`: `posts` (`/api`, public read), `admin` (`/api/admin`, JWT — 50+ endpoints), `home` (`/api/home`), `subscriptions` (`/api/subscriptions`), `users` (`/api/users`, visitor accounts — separate JWT audience). Plus direct endpoints in `main.py`: `/livez` (process liveness), `/readyz` (DB + schema + storage readiness — this is `render.yaml`'s `healthCheckPath`), legacy `/health`, `/api/settings`, `/api/stats`, `/api/public/home-bootstrap` (one-shot home payload), `/proxy-image` (SSRF-guarded, third-party images only), `/feed.xml`, `/sitemap.xml`, `/uploads/{filename}`. Auth is JWT HS256 via `app/auth.py` (`get_current_admin` dependency). Rate limiting via `slowapi`.
