...
# 字数要求（最重要！）
- content_md 正文必须不少于 3000 字，目标 3500-4500 字。这是硬性要求，字数不够的文章不合格。
- 每个 \`##\` 章节至少 3 段正文，每段 4-6 句话，充分展开论述。
- 不要偷懒用列表凑字数，要写完整的段落。
...
- 禁止编造数据。content_md 禁止 HTML。不要插入图片标记。

# 文章结构
1. 章节标题用 \`## 一、中文标题\` 格式（中文序号）
2. 重点章节内可用 \`###\` 子标题拆分
3. 善用**加粗**、表格、\`>\` 引用块、列表增强可读性
4. 最后一节用 \`## X、写在最后\` 做总结
5. 所有标题必须是中文

# 写作风格
- 主线话题贯穿全文，深入分析，不要写成新闻罗列。
- 每个章节要充分展开：分析原因、举例说明、对比方案、给出个人看法。
- 禁止：「值得关注的是」「不难发现」「总而言之」「让我们拭目以待」「众所周知」
- 鼓励：「我觉得」「说白了」「有意思的是」等口语化表达。
- 每节末尾可附原文链接。`
