...
def _build_verify_email(nickname: str, verify_url: str) -> tuple[str, str, str]:
    ...
    safe_nickname = escape(nickname, quote=True)
    ...
    <div style="font-family:Arial,'PingFang SC','Microsoft YaHei',sans-serif;line-height:1.7;color:#0f172a;padding:24px;background:#f8fbff">
      <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:18px;padding:32px;border:1px solid #dbeafe">
        ...
        <p style="font-size:15px;color:#334155;margin:0 0 20px;">你好 {safe_nickname}，点击下方按钮完成邮箱验证后即可评论、点赞。</p>
        <a href="{safe_verify_url}" style="display:inline-block;background:#2563eb;color:#ffffff;text-decoration:none;padding:12px 18px;border-radius:999px;font-weight:700;">验证邮箱</a>
        <p style="margin-top:20px;font-size:13px;color:#64748b;">如果按钮无法点击，请复制以下链接到浏览器打开：<br>{safe_verify_url}</p>
