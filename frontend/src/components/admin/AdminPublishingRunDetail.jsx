        ...
        <div className="space-y-2">
          {items.map((item, index) => (
            <div key={`${item.topic_key || item.title || 'item'}-${index}`} className="rounded-md bg-[var(--bg-surface)] p-3">
              <div className="text-sm font-medium text-[var(--text-primary)]">{item.title || item.topic_key || 'Untitled topic'}</div>
  ...
  return (
    <section data-ui="admin-publishing-run-detail" className="rounded-xl border border-[var(--border-muted)] bg-[var(--bg-surface)] p-5">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <h3 className="text-base font-semibold text-[var(--text-primary)]">Publishing Run Detail</h3>
          <div className="mt-1 text-xs text-[var(--text-faint)]">run_id: {runId}</div>
        </div>
        ...
        <div className="space-y-4">
          <div className="grid gap-3 md:grid-cols-3">
            <div className="rounded-lg bg-[var(--bg-canvas)] p-3 text-center">
              <div className="text-xs text-[var(--text-faint)]">Candidates</div>
              <div className="text-xl font-semibold text-[var(--text-primary)]">{summary.candidate_count ?? 0}</div>
            </div>
            <div className="rounded-lg bg-[var(--bg-canvas)] p-3 text-center">
              <div className="text-xs text-[var(--text-faint)]">Published</div>
              <div className="text-xl font-semibold text-[var(--text-primary)]">{summary.published_count ?? 0}</div>
            </div>
            <div className="rounded-lg bg-[var(--bg-canvas)] p-3 text-center">
              <div className="text-xs text-[var(--text-faint)]">Skipped</div>
              <div className="text-xl font-semibold text-[var(--text-primary)]">{summary.skipped_count ?? 0}</div>
            </div>
          </div>
          <div className="grid gap-4 xl:grid-cols-3">
            <SnapshotSection title="Candidate Topics" items={data.candidate_topics} emptyText="No candidate topics." />
            <SnapshotSection title="Published Topics" items={data.published_topics} emptyText="No published topics." />
            <SnapshotSection title="Skipped Topics" items={data.skipped_topics} emptyText="No skipped topics." />
          </div>
        </div>
      ) : null}
    </section>
