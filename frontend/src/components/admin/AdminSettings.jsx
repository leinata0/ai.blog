...
export default function AdminSettings() {
  ...
  async function loadSettings() {
    try {
      ...
      let links = settings.friend_links || []
  ...
  async function handleAssetUpload(field, event) {
    ...
    try {
      ...
      setMsg('图片已上传并写入地址，请点击下方“保存设置”完成持久化。')
    } catch (err) {
  ...
  function updateFriendLink(index, field, value) {
    setSiteSettings((prev) => ({
      ...prev,
      friend_links: prev.friend_links.map((item, i) => (
        i === index ? { ...item, [field]: value } : item
      )),
    }))
  }

  const isSuccess = msg === '保存成功' || msg.includes('图片已上传并写入地址')
    ...
    <div className="rounded-xl p-6 sm:p-8 space-y-5 bg-[var(--bg-surface)]" style={{ boxShadow: 'var(--card-shadow)' }}>
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs leading-relaxed text-[var(--text-faint)]">
          外链图片链接常因防盗链、图床清理而失效。推荐点击“上传图片”写入本站
          {' '}
          <code className="text-[11px]">/uploads/...</code>
          {' '}
          路径，稳定可用。单张图片最大 10MB。
        </p>
        <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
          <input
            ...
            onChange={(e) => setSiteSettings({ ...siteSettings, avatar_url: e.target.value })}
            className="w-full flex-1 px-4 py-2.5 rounded-lg text-sm outline-none min-w-0"
            style={inputStyle}
          ...
          <input
            ...
            accept="image/*"
            className="hidden"
            onChange={(e) => handleAssetUpload('avatar_url', e)}
          />
          <button
            type="button"
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs leading-relaxed text-[var(--text-faint)]">
          同上，可上传或手动填写 URL；留空则使用头像。单张图片最大 10MB。
        </p>
        <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
          <input
          ...
          <input
            ...
            accept="image/*"
            className="hidden"
            onChange={(e) => handleAssetUpload('hero_image', e)}
          />
          <button
            type="button"
            disabled={assetUploading}
      ...
      <div className="space-y-1">
        ...
        <input
          ...
          onChange={(e) => setSiteSettings({ ...siteSettings, github_link: e.target.value })}
          className="w-full px-4 py-2.5 rounded-lg text-sm outline-none"
          style={inputStyle}
        />
      </div>
        ...
        {(siteSettings.friend_links || []).map((link, i) => (
          <div key={i} className="p-4 rounded-lg border border-[var(--border-muted)] relative">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <input
                value={link.name}
                onChange={(e) => updateFriendLink(i, 'name', e.target.value)}
              ...
              <input
                value={link.url}
                onChange={(e) => updateFriendLink(i, 'url', e.target.value)}
                className="px-3 py-2 rounded-lg text-sm outline-none"
                style={inputStyle}
                placeholder="https://..."
              />
              <input
                ...
                onChange={(e) => updateFriendLink(i, 'description', e.target.value)}
                className="px-3 py-2 rounded-lg text-sm outline-none"
                style={inputStyle}
                placeholder="描述"
              />
              <input
                value={link.avatar}
                onChange={(e) => updateFriendLink(i, 'avatar', e.target.value)}
                className="px-3 py-2 rounded-lg text-sm outline-none"
                style={inputStyle}
                placeholder="头像 URL"
              />
