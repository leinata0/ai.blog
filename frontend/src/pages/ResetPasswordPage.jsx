...
export default function ResetPasswordPage() {
  ...
  return (
    <AuthLayout title="设置一个新的密码" description="验证码只使用一次。重置成功后，其他设备上的旧登录状态会被撤销。" documentTitle="重置密码">
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        <div className="space-y-1.5">
          ...
          <input id="reset-email" name="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} className={inputClass} style={inputStyle} autoComplete="email" spellCheck={false} required />
        </div>
        <div className="space-y-1.5">
          <label htmlFor="reset-code" className="text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>验证码</label>
          <div className="flex gap-2">
            <input id="reset-code" name="code" inputMode="numeric" pattern="[0-9]{6}" maxLength={6} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} className={inputClass} style={inputStyle} placeholder="6 位验证码" autoComplete="one-time-code" spellCheck={false} required />
            <button type="button" onClick={resend} disabled={loading || cooldown > 0} className="min-h-11 w-24 shrink-0 rounded-xl border px-3 text-xs font-semibold disabled:opacity-50" style={{ borderColor: 'var(--border-muted)', color: 'var(--accent)' }}>{cooldown > 0 ? `${cooldown}s 后重发` : '重新发送'}</button>
          </div>
        </div>
        <div className="space-y-1.5">
          <label htmlFor="reset-password" className="text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>新密码</label>
          <input id="reset-password" name="password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} className={inputClass} style={inputStyle} placeholder="至少 8 位" autoComplete="new-password" required />
        </div>
        <div className="space-y-1.5">
